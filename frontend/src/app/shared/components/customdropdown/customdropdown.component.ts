...
import { ChangeDetectorRef, Component, ElementRef, HostListener, Inject, Input, ViewEncapsulation, inject } from '@angular/core';
import { NavigationEnd, Router } from '@angular/router';
import { Store } from '@ngrx/store';
import { createPopper } from '@popperjs/core';
import { getSidebarsize } from '../../../store/layout/layout.selectors';
...
export class CutomDropdownComponent {
  ...
  onDocumentClick(event: Event) {
    if (this._document.documentElement.getAttribute('data-layout') == 'horizontal') {
    ...
    if (this._document.documentElement.getAttribute('data-layout') == 'horizontal') {
      ...
      if (dropdownMenu) {
        if (!dropdownMenu.classList.contains('show')) {
          ...
          this.deactivateParentDropdown(dropdownMenu);
        }
      } else {
        this.initActiveMenu();
      }
  ...
  initActiveMenu() {
    ...
    const ul = this._document.getElementById('navbar-nav');
    if (ul) {
  ...
  activateParentDropdown(item: any) {
    ...
    if (this._document.documentElement.getAttribute('data-layout') == 'vertical') {
  ...
  deactivateParentDropdown(item: any) {
    item.classList.remove('active', 'show');
    item.classList?.add('hidden');
    const parentCollapseDiv = item?.closest('.dropdown-content')?.parentElement;
    if (this._document.documentElement.getAttribute('data-layout') == 'vertical') {
      parentCollapseDiv?.classList.add('hidden');
    }
    const parentDropButton = parentCollapseDiv?.parentElement.parentElement.children[0].querySelector('.dropdown-button');
    parentDropButton?.classList.remove('active', 'show');
  }
}
