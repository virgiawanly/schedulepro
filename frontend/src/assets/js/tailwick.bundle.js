...
(function () {
  ...
  function pluginData() {
    ...
    Array.from(toastExamples).forEach(function (element) {
      element.addEventListener('click', function () {
        ...
        if (isToastVal['data-toast-text']) toastData.text = isToastVal['data-toast-text'].value.toString();
        if (isToastVal['data-toast-gravity']) toastData.gravity = isToastVal['data-toast-gravity'].value.toString();
        if (isToastVal['data-toast-position']) toastData.position = isToastVal['data-toast-position'].value.toString();
        if (isToastVal['data-toast-className']) toastData.className = isToastVal['data-toast-className'].value.toString();
        if (isToastVal['data-toast-duration']) toastData.duration = isToastVal['data-toast-duration'].value.toString();
        if (isToastVal['data-toast-close']) toastData.close = isToastVal['data-toast-close'].value.toString();
        if (isToastVal['data-toast-style']) toastData.style = isToastVal['data-toast-style'].value.toString();
        if (isToastVal['data-toast-offset']) toastData.offset = isToastVal['data-toast-offset'];
    ...
    Array.from(flatpickrExamples).forEach(function (item) {
      if (item.getAttribute('data-provider') == 'flatpickr') {
        ...
        if (isFlatpickerVal['data-inline-date']) {
          (dateData.inline = true), (dateData.defaultDate = isFlatpickerVal['data-default-date'].value.toString());
          dateData.dateFormat = isFlatpickerVal['data-date-format'].value.toString();
        }
        ...
        if (isTimepickerVal['data-time-basic']) {
          (timeData.enableTime = true), (timeData.noCalendar = true), (timeData.dateFormat = 'H:i');
        }
        if (isTimepickerVal['data-time-hrs']) {
          (timeData.enableTime = true), (timeData.noCalendar = true), (timeData.dateFormat = 'H:i'), (timeData.time_24hr = true);
        }
        ...
        if (isTimepickerVal['data-time-inline']) {
          (timeData.enableTime = true), (timeData.noCalendar = true), (timeData.defaultDate = isTimepickerVal['data-time-inline'].value.toString());
  ...
  function drawerSetting() {
    ...
    if (allModalButtons.length > 0 || allDrawerButtons.length > 0) document.body.appendChild(backDropOverlay);
  ...
  function initTooltip() {
    ...
    allTooltipButtons.forEach((element) => {
      ...
      if (element.getAttribute('data-tooltip-placement')) tippyOptions.placement = element.getAttribute('data-tooltip-placement');
      if (element.getAttribute('data-tooltip-content')) tippyOptions.content = element.getAttribute('data-tooltip-content');
      ...
      if (element.getAttribute('data-tooltip-duration')) tippyOptions.duration = element.getAttribute('data-tooltip-duration');
      if (element.getAttribute('data-tooltip-animation')) tippyOptions.animation = element.getAttribute('data-tooltip-animation');
      if (element.getAttribute('data-tooltip-trigger')) tippyOptions.trigger = element.getAttribute('data-tooltip-trigger');
      if (element.getAttribute('data-tooltip-follow-cursor')) tippyOptions.followCursor = element.getAttribute('data-tooltip-follow-cursor');
      if (element.getAttribute('data-tooltip-theme')) tippyOptions.theme = element.getAttribute('data-tooltip-theme');
