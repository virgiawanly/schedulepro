...
if (ckClassicEditor) {
  Array.from(ckClassicEditor).forEach(function () {
    ClassicEditor.create(document.querySelector('.ckeditor-classic')).catch(function (error) {
      console.error(error);
    });
  });
}
  ...
  Array.from(datas).forEach(function (mailData, index) {
    ...
    document.querySelector('#mail-list').innerHTML +=
      ...
      `" class="itemCheckbox size-4 cursor-pointer bg-white border border-slate-200 checked:bg-none dark:bg-zink-700 dark:border-zink-500 rounded-sm appearance-none arrow-none relative after:absolute after:content-['\\eb7b'] after:top-0 after:left-0 after:font-remix after:leading-none after:opacity-0 checked:after:opacity-100 after:text-custom-500 checked:border-custom-500 dark:after:text-custom-500 dark:checked:border-custom-800" >
                <a href="javascript:void(0);" class="transition-all duration-200 ease-linear text-slate-500 hover:text-yellow-500 dark:text-zink-200 dark:hover:text-yellow-500 ` +
      checkStarred +
      `"><lucide-angular name="star" class="size-4"></i></a>
                <a href="javascript:void(0);" class="transition-all duration-200 ease-linear text-slate-500 hover:text-yellow-500 dark:text-zink-200 dark:hover:text-yellow-500"><lucide-angular name="step-forward" class="size-4"></i></a>
            </div>
        </td>
        <td class="relative px-3.5 py-2.5 border-y border-transparent first:pl-0 last:pr-0 text-slate-500 group-[.unread]/mail:text-slate-800 dark:text-zink-200 dark:group-[.unread]/mail:text-zink-50  mailBox">
            <div class="grid items-center grid-cols-12 gap-3">
                <div class="col-span-4 lg:col-span-2">
                    <a href="javascript:void(0);" class="block truncate before:inset-0 before:absolute">` +
      mailData.name +
      `</a>
                </div>
...
if ((document.querySelector('#mail-list .itemCheckbox').checked = true)) {
  console.log('+++ call');
}
  ...
  Array.from(checkboxes).forEach(function (element) {
    element.addEventListener('click', function (event) {
      ...
      if (event.target.closest('li').classList.contains('active')) {
        checkedCount > 0
          ? (document.getElementById('email-topbar-actions').style.display = 'block')
